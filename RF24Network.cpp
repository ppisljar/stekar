...
#else
RF24Network::RF24Network(RF24& _radio, RF24& _radio1)
  : radio(_radio)
  , radio1(_radio1)
  , next_frame(frame_queue)
{
...
void
RF24Network::updateAddress(uint16_t _node_address)
{
...
#if defined ENABLE_NETWORK_STATS
void
RF24Network::failures(uint32_t* _fails, uint32_t* _ok)
{
  *_fails = nFails;
  *_ok = nOK;
}
...
RF24Network::update(void)
{
  ...
  // Incoming Hold prevents data from being read from the radio, preventing
  // incoming payloads from being acked

  if (!(networkFlags & FLAG_BYPASS_HOLDS)) {
    if ((networkFlags & FLAG_HOLD_INCOMING) ||
  ...
  while (radio.isValid() && radio.available(&pipe_num)) {
...
#if defined(ENABLE_DYNAMIC_PAYLOADS)
    if ((frame_size = radio.getDynamicPayloadSize()) <
    ...
    IF_SERIAL_DEBUG(printf_P(PSTR("%lu: MAC Received on %u %s\n\r"), millis(),
                             pipe_num, header->toString()));
    IF_SERIAL_DEBUG(const uint16_t* i = reinterpret_cast<const uint16_t*>(
      ...
      if ((returnSysMsgs && header->type > 127) ||
        ...
        IF_SERIAL_DEBUG_ROUTING(
          printf_P(PSTR("%lu MAC: System payload rcvd %d\n"), millis(),
                   returnVal););
        // if( (header->type < 148 || header->type > 150) && header->type !=
        // NETWORK_MORE_FRAGMENTS_NACK && header->type != EXTERNAL_DATA_TYPE &&
        // header->type!= NETWORK_LAST_FRAGMENT){
        if (header->type != NETWORK_FIRST_FRAGMENT &&
            header->type != NETWORK_MORE_FRAGMENTS &&
            header->type != NETWORK_MORE_FRAGMENTS_NACK &&
...
uint8_t
RF24Network::enqueue(RF24NetworkHeader* header)
{
  bool result = false;
  uint16_t message_size = frame_size - sizeof(RF24NetworkHeader);

  IF_SERIAL_DEBUG(printf_P(PSTR("%lu: NET Enqueue @%x "), millis(),
                           next_frame - frame_queue));
  ...
  if (isFragment) {
    ...
    if (header->type == NETWORK_FIRST_FRAGMENT) {
      // Drop frames exceeding max size and duplicates (MAX_PAYLOAD_SIZE needs
      ...
      if (header->reserved > (MAX_PAYLOAD_SIZE / max_frame_payload_size)) {
        ...
        printf_P(PSTR("Frag frame with %d frags exceeds MAX_PAYLOAD_SIZE or "
                      "out of sequence\n"),
                 header->reserved);
#endif
        frag_queue.header.reserved = 0;
        return false;
      } else if (frag_queue.header.id == header->id &&
                 frag_queue.header.from_node == header->from_node) {
        return true;
      }

      if ((header->reserved * 24) >
          (MAX_PAYLOAD_SIZE - (next_frame - frame_queue))) {
        networkFlags |= FLAG_HOLD_INCOMING;
        radio.stopListening();
      }

      memcpy(&frag_queue, &frame_buffer, 8);
      memcpy(frag_queue.message_buffer,
             frame_buffer + sizeof(RF24NetworkHeader), message_size);

      // IF_SERIAL_DEBUG_FRAGMENTATION( Serial.print(F("queue first, total frags
      // ")); Serial.println(header->reserved); );
      ...
      IF_SERIAL_DEBUG_FRAGMENTATION_L2(
        for (int i = 0; i < frag_queue.message_size;
             i++) { Serial.println(frag_queue.message_buffer[i], HEX); });
      ...
      if (frag_queue.header.reserved == 0 ||
          (header->type != NETWORK_LAST_FRAGMENT &&
           header->reserved != frag_queue.header.reserved) ||
      ...
      memcpy(frag_queue.message_buffer + frag_queue.message_size,
             frame_buffer + sizeof(RF24NetworkHeader), message_size);
      frag_queue.message_size += message_size;
      ...
      if (MAX_PAYLOAD_SIZE - (next_frame - frame_queue) >=
          frag_queue.message_size) {
        memcpy(next_frame, &frag_queue, 10);
        memcpy(next_frame + 10, frag_queue.message_buffer,
               frag_queue.message_size);
      ...
      IF_SERIAL_DEBUG_FRAGMENTATION(
        printf_P(PSTR("Drop frag payload, queue full\n")););
      return false;
...
// Copy the current frame into the frame queue
  ...
  if (message_size + (next_frame - frame_queue) <= MAIN_BUFFER_SIZE) {
    ...
    // Serial.print(next_frame[i],HEX); Serial.print(" : "); }
    // Serial.println(""); );
...
#endif
    // IF_SERIAL_DEBUG_FRAGMENTATION( Serial.print("Enq ");
    // Serial.println(next_frame-frame_queue); );//printf_P(PSTR("enq
    // %d\n"),next_frame-frame_queue); );
...
uint16_t
RF24Network::peek(RF24NetworkHeader& header)
{
  if (available()) {
...
uint16_t
RF24Network::read(RF24NetworkHeader& header, void* message, uint16_t maxlen)
{
  uint16_t bufsize = 0;
  ...
  if (available()) {
    ...
    if (maxlen > 0) {
      ...
      IF_SERIAL_DEBUG(
        uint16_t len = maxlen; printf_P(PSTR("%lu: NET r message "), millis());
...
#endif
    memmove(frame_queue, frame_queue + bufsize + 10 + padding,
            sizeof(frame_queue) - bufsize);
    // IF_SERIAL_DEBUG(printf_P(PSTR("%lu: NET Received
...
/******************************************************************/
bool
RF24Network::multicast(RF24NetworkHeader& header, const void* message,
...
/******************************************************************/
bool
RF24Network::write(RF24NetworkHeader& header, const void* message, uint16_t len)
{
                   ...
                   uint16_t writeDirect)
{
  ...
  // Allows time for requests (RF24Mesh) to get through between failed writes on
  // busy nodes
  ...
  if (len > MAX_PAYLOAD_SIZE) {
    IF_SERIAL_DEBUG(printf("%u: NET write message failed. Given 'len' %d is "
                           "bigger than the MAX Payload size %i\n\r",
                           millis(), len, MAX_PAYLOAD_SIZE););
    return false;
  }
  ...
  IF_SERIAL_DEBUG_FRAGMENTATION(
    printf("%lu: FRG Total message fragments %d\n\r", millis(), fragment_id););
  ...
  while (fragment_id > 0) {
    ...
    if (!ok && retriesPerFrag >= 3) {
      IF_SERIAL_DEBUG_FRAGMENTATION(
        printf("%lu: FRG TX with fragmentID '%d' failed after %d fragments. "
               "Abort.\n\r",
               millis(), fragment_id, msgCount););
      break;
    }
...
// Message was successful sent
  ...
  IF_SERIAL_DEBUG_FRAGMENTATION(
    printf("%u: FRG total message fragments sent %i. \n", millis(), msgCount););
  if (fragment_id > 0) {
    txTime = millis();
    return false;
  }
...
bool
RF24Network::_write(RF24NetworkHeader& header, const void* message,
                    uint16_t len, uint16_t writeDirect)
{
  ...
  if (len) {
    ...
    IF_SERIAL_DEBUG(
      uint16_t tmpLen = len; printf_P(PSTR("%lu: NET message "), millis());
      const uint8_t* charPtr = reinterpret_cast<const uint8_t*>(message);
      while (tmpLen--) {
  ...
  if (writeDirect != 070) {
    uint8_t sendType = USER_TX_TO_LOGICAL_ADDRESS; // Payload is multicast to
                                                   // the first node, and routed
                                                   // normally to the next

    if (header.to_node == 0100) {
      sendType = USER_TX_MULTICAST;
    }
    if (header.to_node == writeDirect) {
      sendType = USER_TX_TO_PHYSICAL_ADDRESS; // Payload is multicast to the
                                              // first node, which is the
                                              // recipient
    }
...
bool RF24Network::write(uint16_t to_node,
                        uint8_t directTo) // Direct To: 0 = First Payload,
                                          // standard routing, 1=routed payload,
                                          ...
                                          // 3=directRoute to Route
{
  ...
  IF_SERIAL_DEBUG(
    printf_P(PSTR("%lu: MAC Sending to 0%o via 0%o on pipe %x\n\r"), millis(),
             to_node, conversion.send_node, conversion.send_pipe));
  ...
  if (ok && conversion.send_node != to_node &&
    ...
    while (update() != NETWORK_ACK) {
      ...
      if (millis() - reply_time > routeTimeout) {
        ...
        IF_SERIAL_DEBUG_ROUTING(
          printf_P(
            PSTR("%lu: MAC Network ACK fail from 0%o via 0%o on pipe %x\n\r"),
            millis(), to_node, conversion.send_node, conversion.send_pipe););
...
bool
RF24Network::write_to_pipe(uint16_t node, uint8_t pipe, bool multicast)
{
  bool ok = false;
  ...
  /*
    IF_SERIAL_DEBUG(printf_P(PSTR("%lu: MAC Sent on %lx
...
const char*
RF24NetworkHeader::toString(void) const
{
  static char buffer[45];
  // snprintf_P(buffer,sizeof(buffer),PSTR("id %04x from 0%o to 0%o type
  // %c"),id,from_node,to_node,type);
  sprintf_P(buffer, PSTR("id %u from 0%o to 0%o type %d"), id, from_node,
            to_node, type);
  return buffer;
}
...
bool
RF24Network::is_direct_child(uint16_t node)
{
  bool result = false;
...
bool
RF24Network::is_descendant(uint16_t node)
{
...
void
RF24Network::setup_address(void)
{
...
bool
RF24Network::is_valid_address(uint16_t node)
{
  bool result = true;
...
#if defined(RF24NetworkMulticast)
void
RF24Network::multicastLevel(uint8_t level)
{
...
pipe_address(uint16_t node, uint8_t pipe)
{
  ...
  while (dec) {
    ...
    if (pipe != 0 || !node)
#endif
      out[count] = address_translation[(dec % 8)]; // Convert our decimal values
                                                   // to octal, translate them
                                                   // to address bytes, and set
                                                   // our address
...
bool
RF24Network::sleepNode(unsigned int cycles, int interruptPin)
{
...
void
RF24Network::setup_watchdog(uint8_t prescalar)
{
...
#if defined(__AVR_ATtiny25__) || defined(__AVR_ATtiny45__) ||                  \
  defined(__AVR_ATtiny85__)
  WDTCR = _BV(WDCE) | _BV(WDE); // Write the WD Change enable bit to enable
                                // changing the prescaler and enable system
                                // reset
  WDTCR = _BV(WDCE) | wdtcsr | _BV(WDIE); // Write the prescalar bits (how long
                                          // to sleep, enable the interrupt to
                                          // wake the MCU
#else
  WDTCSR = _BV(WDCE) | _BV(WDE); // Write the WD Change enable bit to enable
                                 // changing the prescaler and enable system
                                 // reset
  WDTCSR = _BV(WDCE) | wdtcsr | _BV(WDIE); // Write the prescalar bits (how long
                                           // to sleep, enable the interrupt to
                                           // wake the MCU
#endif
}
